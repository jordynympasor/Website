<?php 
    ...
    $query= "DELETE FROM voucher where number='$account'";
    $query1="SELECT * FROM voucher where number='$account'";
    $result=mysqli_query($link,$query1);
    $row=mysqli_fetch_array($result);
    if (isset($_POST["yes"])) {
       mysqli_query($link,$query);
       header("Location: viewaccountvoucher2.php?id=".$row['ownernumber']."");
    }
?>
<!doctype html>
<html>
...
<link rel="stylesheet" href="https://unpkg.com/purecss@1.0.1/build/pure-min.css" integrity="sha384-oAOxQR6DkCoMliIh8yFnu25d7Eq/PHS21PClpwjOTeU2jRSq11vu66rf90/cZr47" crossorigin="anonymous">
    ...
    <script> function redirect() { location.replace("viewaccountvoucher2.php?id=<?php   $account=$_GET['id'];
                                                                                    $query1="SELECT * FROM voucher where number='$account'";
                                                                                    $result=mysqli_query($link,$query1);
                                                                                    $row=mysqli_fetch_array($result);
                                                                                    echo $row['ownernumber'] ?>");}</script>
...
<nav class="navbar navbar-expand-lg navbar-light bg-light" style="top: 0; width: 100%;"> <a class="navbar-brand" href="superuserpage.php"><img src="58779115_829177404107193_6770553374166220800_n.jpg" style="border-radius: 50%;"  width="70"></a>
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent1">
    <ul class="navbar-nav mr-auto">
        <li class="nav-item"> <a class="nav-link" href="viewallaccounts2.php">View All Accounts</a> </li>
        </ul>
        <ul class="navbar-nav">
...
<center>    <h5>Are you sure you would like to delete <?php $account=$_GET['id'];
                                        ...
                                        $row = mysqli_fetch_array($result);
                                        echo  $row["client_name"] ;?>'s Voucher details?    </h5>
    <div>
    <style scoped>
